...
export default function AnalyticsDashboardPanel() {
  ...
  const fetchDashboards = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/dashboards?action=list&${params}`)
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch("/api/admin/dashboards?action=stats")
      const data = await response.json()
  ...
  const handleDeleteDashboard = async (dashboardId: string) => {
    ...
    try {
      const response = await fetch(`/api/admin/dashboards?action=delete&id=${dashboardId}`, {
        method: "DELETE"
      })
