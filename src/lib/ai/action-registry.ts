...
export interface ActionEntry {
  ...
  /** Generation settings verified from LoRA creator examples */
  gen_settings?: {
    steps: number;
    cfg: number;
    sampler: string;
...
export const ACTION_REGISTRY: Record<string, ActionEntry> = {
  ...
  sfw_clothed: {
    ...
    gen_settings: { steps: 8, cfg: 1.2, sampler: 'euler' },
    status: 'verified',
  ...
  nude_pose: {
    ...
    gen_settings: { steps: 8, cfg: 1.0, sampler: 'euler' },
    status: 'verified',
  ...
  dildo_pussy: {
    ...
    image_lora: 'female_anatomy_dildo_riding_k3nk.safetensors',
    image_lora_weight: 1.0,
    reference_category: '01_dildo_pussy',
    reference_tags: ['dildo', 'insertion', 'pussy', 'riding', 'full_body'],
    ...
    gen_settings: { steps: 12, cfg: 1.0, sampler: 'euler' },
    status: 'verified',
    notes: 'K3nk LoRA — VERIFIED PERFECTION by Steve 2026-08-14: "riding the Dildo and it looks Great". Replaces FK (retired — locked to sitting pose, fought skeleton guidance).',
  },
  ...
  dildo_anal: {
    ...
    prompt_fragment: 'dildo inside her asshole, her hand reaching behind holding it',
    is_nsfw: true,
    use_controlnet: true,
    controlnet_strength: 0.9,
    target_hole: 'ass',
    default_expression: 'biting her lip, eyes squinting, mix of pleasure and discomfort',
    // Creator: steps=6, CFG=1, Euler (from model examples)
  ...
  // Dildo riding — uses K3nk LoRA which Steve confirmed produces riding pose
  // (different from FK's sitting pose). Creator settings: 12 steps, CFG 1, LCM.
  dildo_riding: {
    action_id: 'dildo_riding',
    aliases: ['riding a dildo', 'riding dildo', 'on top of dildo', 'squatting on dildo', 'dildo ride'],
    image_lora: 'female_anatomy_dildo_riding_k3nk.safetensors',
    image_lora_weight: 1.0,
    reference_category: '01_dildo_pussy',
    reference_tags: ['dildo', 'riding', 'squatting', 'pussy'],
    prompt_fragment: 'riding a dildo, dildo inside her vagina, her hand steadying herself',
    is_nsfw: true,
    use_controlnet: true,
    controlnet_strength: 0.45,
    target_hole: 'pussy',
    default_expression: 'eyes rolled back, mouth open gasping, intense pleasure',
    gen_settings: { steps: 12, cfg: 1.0, sampler: 'lcm' },
    status: 'verified',
    notes: 'K3nk LoRA — Steve confirmed: "Looks great she is riding a dildo and its Holly photorealistic". Produces DIFFERENT pose from FK (riding instead of sitting).',
  },

  // Dildo from behind — bent over using dildo
  dildo_from_behind: {
    action_id: 'dildo_from_behind',
    aliases: ['dildo from behind', 'dildo bent over', 'dildo doggy', 'using dildo from behind'],
    image_lora: 'female_anatomy_dildo_riding_k3nk.safetensors',
    image_lora_weight: 1.0,
    reference_category: '17_bent_over',
    reference_tags: ['dildo', 'bent_over', 'behind', 'pussy'],
    prompt_fragment: 'dildo inside her vagina from behind, her hand reaching between her legs',
    is_nsfw: true,
    use_controlnet: true,
    controlnet_strength: 0.45,
    target_hole: 'pussy',
    default_expression: 'biting her lip, eyes squinting, looking back over shoulder',
    gen_settings: { steps: 12, cfg: 1.0, sampler: 'euler' },
    status: 'untested',
    notes: 'FK dildo + bent_over reference photos = different pose. Uses bent_over refs for body position.',
  },

  // ─── Wet / Shower ─────────────────────────────────────────────
  wet_shower: {
    action_id: 'wet_shower',
    aliases: ['wet', 'shower', 'bath', 'pool', 'swimming', 'soaking wet', 'wet skin'],
    image_lora: 'wet_babes.safetensors',
    image_lora_weight: 0.7,
    reference_category: '',
    reference_tags: ['wet', 'water', 'shower'],
    prompt_fragment: 'Wet_Babe, soaking wet skin, dripping water droplets on body, wet hair',
    is_nsfw: true,
    use_controlnet: false,
    controlnet_strength: 0,
    default_expression: 'relaxed content expression',
    gen_settings: { steps: 8, cfg: 1.0, sampler: 'euler' },
    status: 'verified',
    notes: 'Steve confirmed: "looks good Keep this one for when Holly is creating a shower or wet look".',
  },

  // ─── Teasing ─────────────────────────────────────────────────
  oops_no_panties: {
    action_id: 'oops_no_panties',
    aliases: ['no panties', 'upskirt', 'oops', 'wardrobe malfunction', 'no underwear'],
    image_lora: 'oops_no_panties.safetensors',
    image_lora_weight: 0.7,
    reference_category: '',
    reference_tags: ['panties', 'teasing', 'upskirt'],
    prompt_fragment: 'pubis, bare pubis, wearing a short skirt with no panties',
    is_nsfw: true,
    use_controlnet: false,
    controlnet_strength: 0,
    default_expression: 'shy blushing expression',
    gen_settings: { steps: 8, cfg: 1.0, sampler: 'euler' },
    status: 'untested',
    notes: 'Trigger: "pubis, bare pubis". SFW-adjacent teasing. Needs clothing in prompt.',
  },
  ...
  fingering: {
    ...
    // INSERT Kit creator: CFG 1, 12 steps, LCM (from model examples)
    gen_settings: { steps: 12, cfg: 1.0, sampler: 'lcm' },
    status: 'verified',
  ...
  masturbation: {
    ...
    prompt_fragment: 'fingers inside her pussy, her hand between her legs, masturbating',
    is_nsfw: true,
    use_controlnet: true,
    controlnet_strength: 0.45,
    target_hole: 'pussy',
