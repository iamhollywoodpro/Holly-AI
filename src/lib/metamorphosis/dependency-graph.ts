...
import type { ParsedFile } from './codebase-parser';
...
export interface FileDependency {
  ...
  type: 'direct' | 'indirect'; // Direct import or transitive dependency
}
...
export interface DependencyNode {
  ...
  usedBy: string[];          // Files that depend on this
  layer: 'api' | 'services' | 'database' | 'ui' | 'lib' | 'types' | 'utils';
...
export interface DependencyGraph {
  ...
  criticalPaths: string[][]; // Paths through critical dependencies
  circularDependencies: string[][]; // Circular dependency chains
  impactAnalysis: {
    file: string;
    directImpact: string[];  // Files directly affected by changes
    totalImpact: string[];   // All files affected (including transitive)
  }[];
}
...
export class DependencyGraphGenerator {
   ...
   */
  private async analyzeDependencies(filePath: string): Promise<DependencyNode> {
  ...
  private determineLayer(filePath: string): DependencyNode['layer'] {
    if (filePath.includes('app/api/') || filePath.includes('pages/api/')) return 'api';
    if (filePath.includes('src/lib/') || filePath.includes('lib/')) return 'lib';
    if (filePath.includes('src/services/') || filePath.includes('services/')) return 'services';
    if (filePath.includes('prisma/') || filePath.includes('src/lib/db')) return 'database';
    if (filePath.includes('src/components/') || filePath.includes('components/')) return 'ui';
    if (filePath.includes('src/types/') || filePath.includes('types/')) return 'types';
    return 'utils';
  }
   ...
   */
  private buildEdges(nodes: DependencyNode[]): DependencyGraph['edges'] {
   ...
   */
  private identifyCriticalPaths(nodes: DependencyNode[]): string[][] {
   ...
   */
  private findPath(from: string, to: string, nodes: DependencyNode[]): string[] | null {
   ...
   * Detect circular dependencies
   */
  private detectCircularDependencies(nodes: DependencyNode[]): string[][] {
    const cycles: string[][] = [];
    const visited = new Set<string>();
   ...
   */
  private buildImpactAnalysis(nodes: DependencyNode[]): DependencyGraph['impactAnalysis'] {
   ...
   */
  private calculateTotalImpact(file: string, nodes: DependencyNode[]): string[] {
   ...
   */
  async getFileDependencies(filePath: string): Promise<FileDependency | null> {
    const relativePath = path.relative(this.projectRoot, filePath);
