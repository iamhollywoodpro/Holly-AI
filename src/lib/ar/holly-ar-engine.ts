...
export interface ARAnalysisRequest {
  userId:          string;       // User performing the analysis
...
export async function runARAnalysis(req: ARAnalysisRequest): Promise<HollyARResult> {
  ...
  // Step 2a: Look for relevant past A&R critiques (Phase 4 Taste Matrix)
  let pastCritiquesContext = '';
  try {
    const { semanticSearch } = await import('@/lib/memory/semantic-memory');
    const pastCritiques = await semanticSearch(req.userId, trackInfo + ' ' + (req.genre || ''), {
      limit: 3,
      threshold: 0.45,
    ...
    if (pastCritiques.length > 0) {
      const critiqueBlock = pastCritiques
        .map(c => `[Past Analysis - ${c.createdAt.toLocaleDateString()}]: ${c.content.substring(0, 500)}...`)
        .join('\n\n');
      pastCritiquesContext = `\n\n## YOUR PREVIOUS FEEDBACK (Historical Context):\n${critiqueBlock}\n\nINSTRUCTION: Compare this new track to the user's previous work above when relevant. Note improvements or regressions in production and writing.`;
    }
  } catch (memErr) {
    console.warn('[A&R Engine] Failed to fetch historical critiques:', memErr);
  }

  const prompt = `${trackInfo}
${auraContext}${lyricsContext}${pastCritiquesContext}${userQ}
...
HOLLY — Senior A&R`;
  ...
  // Step 5: Store results in Semantic Memory (Background) — Phase 4 Taste Matrix
  if (req.userId) {
    import('@/lib/memory/semantic-memory').then(async ({ storeMemory }) => {
      await storeMemory({
        userId: req.userId,
        content: `A&R Analysis for "${artistLabel}"\nRating: ${billboardRating.overall}/100\nDecision: ${parsed.signingDecision}\nKey Strengths: ${parsed.strengths.join(', ')}\nConcerns: ${parsed.concerns.join(', ')}\nLetter: ${parsed.arLetter}`,
        type: 'music_critique',
        metadata: {
          trackTitle: req.trackTitle,
          artistName: req.artistName,
          overallRating: billboardRating.overall,
          tier: billboardRating.tier
        }
      }).catch(() => {});
    }).catch(err => console.error('[A&R Engine] Memory store failed:', err));
  }
