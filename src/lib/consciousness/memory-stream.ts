...
export class MemoryStream {
  ...
  private async findRelatedExperiences(experience: Experience): Promise<Experience[]> {
    try {
      const searchContent = typeof experience.content === 'object'
        ? `${(experience.content as any).what || ''} ${(experience.content as any).context || ''} ${(experience.content as any).outcome || ''}`
        : String(experience.content || '');
      if (searchContent.length > 5) {
        const { semanticSearch } = await import('@/lib/memory/semantic-memory');
        const results = await semanticSearch(this.userId, searchContent, { limit: 10, threshold: 0.4 });

        if (results.length > 0) {
          const matchIds = results.filter(r => r.id !== experience.id).map(r => r.id);
          if (matchIds.length > 0) {
            const matched = await this.db.hollyExperience.findMany({
              where: {
                userId: this.userId,
                id: { in: matchIds },
              },
            ...
            if (matched.length > 0) return matched.map(exp => this.prismaToExperience(exp));
          }
        }
      }
  ...
  async searchMemories(query: string, limit: number = 10): Promise<Experience[]> {
    try {
      ...
      const results = await semanticSearch(this.userId, query, { limit, threshold: 0.3 });
      ...
      if (results.length > 0) {
        ...
        const matched = experiences.filter(exp => {
          const contentObj = typeof exp.content === 'object' ? (exp.content as Record<string, unknown>) : {};
          const searchable = [
            String(contentObj.what || ''),
            String(contentObj.context || ''),
            String(contentObj.outcome || ''),
            exp.type || '',
            ...(Array.isArray(contentObj.actions) ? contentObj.actions as string[] : []),
          ].join(' ').toLowerCase();
        ...
        return experiences.slice(0, limit).map(exp => this.prismaToExperience(exp));
      }

      const queryLower = query.toLowerCase();
      const experiences = await this.db.hollyExperience.findMany({
        where: { userId: this.userId, significance: { gte: 0.3 } },
        orderBy: { significance: 'desc' },
        take: limit * 3,
      });
      ...
      return matched.slice(0, limit).map(exp => this.prismaToExperience(exp));
    } catch (error) {
