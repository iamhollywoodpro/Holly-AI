...
export async function POST(req: NextRequest) {
  try {
    ...
    const { scope, scopePath, scopeName, metrics, debtItems } = body;
    ...
    const createdDebtItems = [];
    if (Array.isArray(debtItems) && debtItems.length > 0) {
      for (const item of debtItems) {
        const debt = await prisma.technicalDebt.create({
          data: {
            debtType: item.debtType || 'code_smell',
            category: item.category || 'maintainability',
            filePath: item.filePath || scopePath || '',
            fileName: item.fileName || '',
            lineNumbers: item.lineNumbers || [],
            module: item.module || null,
            title: item.title || 'Unnamed debt item',
            description: item.description || '',
            impact: item.impact || '',
            effortMinutes: item.effortMinutes || 0,
            severity: item.severity || 'medium',
            priority: item.priority || 5,
            affectedFeatures: item.affectedFeatures || [],
            userImpact: item.userImpact || null,
            businessCost: item.businessCost || null,
            recommendation: item.recommendation || '',
            suggestedApproach: item.suggestedApproach || null,
            status: 'identified',
          }
        });
        createdDebtItems.push(debt);
      }
    }
    ...
    return NextResponse.json({
      success: true,
      measurement,
      debtItemsCreated: createdDebtItems.length
    });
