...
export async function GET(request: NextRequest) {
  try {
    ...
    // Action: Get stats
    if (action === 'stats') {
      const totalDashboards = await prisma.analyticsDashboard.count({
        where: { OR: [{ createdBy: clerkUserId }, { isPublic: true }] }
      });
      const publicDashboards = await prisma.analyticsDashboard.count({
        where: { isPublic: true }
      });
      const favoriteDashboards = await prisma.analyticsDashboard.count({
        where: { OR: [{ createdBy: clerkUserId }, { isPublic: true }], favorite: true }
      });

      return NextResponse.json({
        success: true,
        data: { total: totalDashboards, public: publicDashboards, favorites: favoriteDashboards }
      });
    }
