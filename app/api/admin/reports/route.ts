...
export async function GET(request: NextRequest) {
  try {
    ...
    if (action === 'stats') {
      ...
      const scheduledReports = await prisma.customReport.count({
        where: { OR: [{ createdBy: clerkUserId }, { isPublic: true }], isScheduled: true }
      });
      const publicReports = await prisma.customReport.count({
        where: { isPublic: true }
      });

      return NextResponse.json({
        success: true,
        data: { total: totalReports, scheduled: scheduledReports, public: publicReports }
      });
    }
