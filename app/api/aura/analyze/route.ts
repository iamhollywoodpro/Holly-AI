...
Respond ONLY with valid JSON:
{
  ...
  "model_version": "AURA-v3.0"
    ...
    const features = (result.features || {}) as Record<string, number>;
    const scores = (result.scores || {}) as Record<string, number>;
    const hitFactor = (result.hit_factor ?? 0) as number;
    ...
    if (analysis) {
      await prisma.musicAnalysis.update({
        ...
        data: {
          ...
          hitScore: hitFactor / 10,
          productionScore: scores.audio / 10,
          marketPotential: hitFactor >= 75 ? 'high' : hitFactor >= 60 ? 'medium' : 'low',
        },
      });
    }
