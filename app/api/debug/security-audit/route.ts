...
const CREATOR_EMAIL = 'iamhollywoodpro@gmail.com';
const CREATOR_CLERK_PREFIX = 'user_2h';
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!clerkUserId.startsWith(CREATOR_CLERK_PREFIX)) {
      const dbUser = await prisma.user.findUnique({
        where: { clerkUserId },
        select: { email: true },
      });
      if (!dbUser || dbUser.email !== CREATOR_EMAIL) {
