...
export async function GET() {
  try {
    ...
    const [
      ...
      safeQuery(() => prisma.hollyIdentity.findFirst({
        where: { userId: uid },
        select: {
      ...
      safeQuery(() => prisma.emotionalState.findMany({
        where: { userId: uid },
      ...
      safeQuery(() => prisma.learningPattern.findMany({
      ...
      safeQuery(() => prisma.evolutionProposal.findMany({
      ...
      safeQuery(() => prisma.learningEvent.groupBy({
        by: ['type'],
        where: { userId: uid, timestamp: { gte: sevenDaysAgo } },
        _count: { id: true },
      }), []),

      safeQuery(() => prisma.tasteProfile.findFirst({
        where: { userId: uid },
        select: {
          tone: true,
          verbosity: true,
      ...
      safeQuery(() => prisma.hollyGoal.findMany({
      ...
      safeQuery(() => prisma.learningEvent.count({ where: { userId: uid } }), 0),
    ]);
