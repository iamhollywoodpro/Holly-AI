...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  'Storage':      'bg-blue-500/20 text-blue-300',
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-400">
          Connect HOLLY to your favourite services — {liveCount} live, {integrations.length - liveCount} coming soon
        </p>
      </div>
  ...
  return (
    <motion.div
      ...
      className={`relative flex flex-col p-5 rounded-2xl border transition-all duration-200 ${
        isConnected
          ? 'bg-gray-800/60 border-green-500/30 shadow-lg shadow-green-500/5'
          : isComingSoon
          ? 'bg-gray-800/20 border-gray-700/20'
          : 'bg-gray-800/50 border-gray-700/40 hover:border-gray-600/60 hover:bg-gray-800/70'
      }`}
    >
      {/* Category pill */}
      ...
      <div className="mb-3">
        {isConnected ? (
          ...
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-gray-700/40 text-gray-500 rounded-full text-xs font-medium">
            Coming Soon
          </span>
        ) : (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-gray-700/40 text-gray-400 rounded-full text-xs font-medium">
            Not Connected
          </span>
        )}
      </div>
      ...
      {/* Description */}
      <p className={`text-sm leading-relaxed mb-4 flex-1 ${isComingSoon ? 'text-gray-600' : 'text-gray-400'}`}>
        {svc.description}
      </p>

      {/* Action button */}
      {loading ? (
        <button disabled className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-gray-700/40 text-gray-500 text-sm cursor-not-allowed">
        ...
        <button
          ...
          className="w-full px-4 py-2.5 rounded-xl bg-gray-700/60 hover:bg-red-500/20 hover:border-red-500/30 border border-transparent text-sm text-gray-300 hover:text-red-300 font-medium transition-all"
        >
          Disconnect
        </button>
      ) : isDisconnected ? (
        <button
          ...
          className={`w-full px-4 py-2.5 rounded-xl bg-gradient-to-r ${svc.gradient} hover:opacity-90 active:scale-[0.98] text-white text-sm font-semibold transition-all shadow-lg`}
        >
          Connect {svc.name}
        </button>
      ) : (
        <button disabled className="w-full px-4 py-2.5 rounded-xl bg-gray-800/40 text-gray-600 text-sm cursor-not-allowed">
          Coming Soon
        </button>
      )}
    </motion.div>
