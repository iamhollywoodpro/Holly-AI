        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 rounded-full bg-gradient-to-br from-[#66CCCC] to-[#C7B8EA] flex items-center justify-center">
                <span className="text-xs font-bold">H</span>
              </div>
              <span className="font-semibold text-white">Generation Studio</span>
              <span className="text-xs bg-[#66CCCC]/40 text-[#3DAF76] px-2 py-0.5 rounded-full border border-[#66CCCC]/50">
                Phase 11
              </span>
            </div>
          </div>
          ...
          {TABS.map(tab => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${
                activeTab === tab.id
                  ? 'bg-gradient-to-r from-[#66CCCC] to-[#C7B8EA] text-white shadow-lg'
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full accent-[#66CCCC]"
                    />
                  ...
                  <label className="flex items-center gap-3 cursor-pointer group">
                    <div
                      ...
                      className={`relative w-11 h-6 rounded-full transition-colors ${
                        mvGenerateVideo ? 'bg-[#66CCCC]' : 'bg-gray-700'
                      }`}
                    >
                      ...
                      {AV_SYNC_MODES.map(mode => (
                        <button
                          ...
                          className={`p-3 rounded-xl border text-left transition-all ${
                            avSyncMode === mode.id
                              ? 'border-[#66CCCC] bg-[#66CCCC]/20 text-white'
          ...
          <div>
            <AnimatePresence mode="wait">
              {isGenerating && (
                ...
                >
                  <div className="w-16 h-16 rounded-full border-2 border-[#66CCCC] border-t-transparent animate-spin" />
...
function LoadingScreen() {
  return (
    <div className="min-h-screen bg-gray-950 flex items-center justify-center">
      <div className="w-10 h-10 rounded-full border-2 border-[#66CCCC] border-t-transparent animate-spin" />
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      ...
      <textarea
        ...
        rows={rows}
        className="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-white placeholder-gray-600 text-sm resize-none focus:outline-none focus:border-[#66CCCC] transition-colors"
      />
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      ...
      <input
        ...
        onChange={e => onChange(e.target.value)}
        placeholder={placeholder}
        className="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-2.5 text-white placeholder-gray-600 text-sm focus:outline-none focus:border-[#66CCCC] transition-colors"
      />
    </div>
  );
}
  ...
  return (
    <div className="space-y-2">
      ...
      <select
        value={value}
        onChange={e => onChange(e.target.value)}
        className="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:border-[#66CCCC] transition-colors appearance-none"
      >
        {options.map(o => (
...
function GenerateButton({ onClick, loading, disabled, label, icon }: {
  ...
  return (
    <button
      ...
      className="w-full py-3.5 bg-gradient-to-r from-[#66CCCC] to-[#C7B8EA] hover:from-[#3DAF76] hover:to-[#C7B8EA] disabled:from-gray-700 disabled:to-gray-700 disabled:text-gray-500 text-white font-semibold rounded-xl transition-all flex items-center justify-center gap-2 shadow-lg"
    >
      {loading ? (
        <>
      ...
      <div className="p-4 bg-gray-900/60 rounded-xl border border-gray-800 space-y-2">
        ...
        {result.url && (
          <a
            ...
            className="text-xs text-[#3DAF76] hover:text-[#66CCCC] underline block"
          >
            Open in new tab ↗
          </a>
        )}
...
function MusicVideoResultView({ result }: { result: MusicVideoResult }) {
  ...
  return (
    <div className="space-y-4">
      <div className="p-4 bg-gradient-to-br from-[#66CCCC]/30 to-[#C7B8EA]/20 rounded-2xl border border-[#66CCCC]/40">
          ...
          {result.scenes.map((scene, i) => (
            <button
              ...
              className={`flex-shrink-0 w-20 h-20 rounded-xl border overflow-hidden transition-all ${
                activeScene === i ? 'border-[#66CCCC] scale-105' : 'border-gray-700 opacity-70 hover:opacity-100'
              }`}
            >
              {scene.imageUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
