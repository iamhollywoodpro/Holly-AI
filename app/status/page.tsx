...
export default function StatusPage() {
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      case 'healthy': return { color: 'text-[#66CCCC]', bg: 'bg-[#66CCCC]/10', border: 'border-[#66CCCC]/30', icon: <CheckCircle className="w-5 h-5" />, label: 'Nominal Operational' };
      case 'degraded': return { color: 'text-[#C7B8EA]', bg: 'bg-[#C7B8EA]/10', border: 'border-[#C7B8EA]/30', icon: <AlertTriangle className="w-5 h-5" />, label: 'Degraded Cognition' };
      case 'critical': return { color: 'text-[#C7B8EA]', bg: 'bg-[#C7B8EA]/10', border: 'border-[#C7B8EA]/30', icon: <XCircle className="w-5 h-5" />, label: 'Critical Protocol Failure' };
  ...
  const integrationLabels: Record<string, { label: string; icon: React.ReactNode; category: string }> = {
    ...
    nvidia_tts: { label: 'NVIDIA Magpie TTS', icon: <Music className="w-4 h-4" />, category: 'media' },
  ...
  return (
    <div className="min-h-screen bg-[#0A0908] text-[#F5F0E8]">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-black text-[#66CCCC] uppercase tracking-[0.2em] mb-1">Nexus Status</h1>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <button
            onClick={checkHealth}
            disabled={loading}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-[#66CCCC] border border-[#66CCCC]/20 rounded-lg hover:bg-[#66CCCC]/10 disabled:opacity-50 transition-colors uppercase font-bold tracking-tighter"
          >
        ...
        {!health ? (
          ...
          <div className="space-y-6">
            <div className={`p-5 rounded-xl border-2 ${getStatusConfig(health.health).bg} ${getStatusConfig(health.health).border}`}>
              <div className="flex items-center gap-3">
                ...
                <div className="ml-auto text-right">
                  <p className="text-2xl font-black text-[#66CCCC]">{health.summary.activeAiProviders}</p>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-[#12110F] border border-[#66CCCC]/10 rounded-xl p-4 text-center">
                <p className="text-2xl font-black text-[#66CCCC]">{health.summary.activeAiProviders}</p>
                <p className="text-[10px] text-[#8C8476] uppercase font-bold">AI Providers</p>
              </div>
              <div className="bg-[#12110F] border border-[#66CCCC]/10 rounded-xl p-4 text-center">
                <p className="text-2xl font-black text-[#66CCCC]">{health.summary.configuredIntegrations}</p>
                <p className="text-[10px] text-[#8C8476] uppercase font-bold">Integrations</p>
              </div>
              <div className="bg-[#12110F] border border-[#66CCCC]/10 rounded-xl p-4 text-center">
                <p className="text-2xl font-black text-[#66CCCC]">{health.system.memoryUsageMB}MB</p>
            ...
            <div className="sdi-glass border border-white/5 rounded-2xl p-5 transition-all">
              ...
              >
                ...
                <span className="text-xs text-[#66CCCC] hover:text-white transition-colors font-mono">
                  {showAiProviders ? '[-]' : '[+]'}
                </span>
              </button>
              ...
              {showAiProviders && (
                <div className="space-y-2 mt-4">
                  {Object.entries(health.providers).map(([key, active]) => {
                    ...
                    return (
                      <div key={key} className={`flex items-center gap-3 p-3 rounded-lg ${active ? 'bg-[#66CCCC]/5 border border-[#66CCCC]/20' : 'bg-[#1E1B18]/30 border border-[#1E1B18]/30'}`}>
                        <span className={active ? 'text-[#66CCCC]' : 'text-[#5C564D]'}>{info.icon}</span>
                        <span className={`text-sm flex-1 font-medium ${active ? 'text-[#F5F0E8]' : 'text-[#5C564D]'}`}>{info.label}</span>
                        {active ? (
                          <span className="text-[10px] text-[#66CCCC] flex items-center gap-1 uppercase font-black tracking-tighter"><CheckCircle className="w-3 h-3" /> Active</span>
                        ) : (
            ...
            <div className="sdi-glass border border-white/5 rounded-2xl p-5 transition-all">
              ...
              >
                <span className="flex items-center gap-2">
                  <Database className="w-4 h-4 text-[#66CCCC]" />
                  ...
                  {Object.entries(health.integrations).map(([key, active]) => {
                    ...
                    return (
                      <div key={key} className={`flex items-center gap-2.5 p-2.5 rounded-lg ${active ? 'bg-[#12110F]/60 border border-white/5' : 'bg-[#12110F]/20 opacity-50'}`}>
                        ...
                        {active ? (
                          <span className="w-2 h-2 rounded-full bg-[#66CCCC] shadow-[0_0_5px_#66CCCC]" />
                        ) : (
            ...
            <div className="sdi-glass border border-white/5 rounded-2xl p-5 transition-all">
              ...
              >
                <span className="flex items-center gap-2">
                  <Activity className="w-4 h-4 text-[#66CCCC]" />
                  System Info
                </span>
                <span className="text-xs text-[#66CCCC] hover:text-white transition-colors font-mono">
                  {showSystemInfo ? '[-]' : '[+]'}
