...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    if (name === "generate_image") {
      ...
      // The <img> tag in the frontend fetches the image directly from Pollinations.
      return text(
    ...
    if (name === "proactive_insights") {
      try {
        ...
        const insights = (body.insights || []).map((i) =>
        ...
        if (body.patterns) {
          result += `\n\n📊 Detected Patterns:`;
          if (body.patterns.topics?.length) result += `\n  Topics: ${body.patterns.topics.map((p) => `${p.pattern} (${p.frequency}x)`).join(', ')}`;
          if (body.patterns.emotions?.length) result += `\n  Emotions: ${body.patterns.emotions.map((p) => `${p.pattern} (${p.frequency}x)`).join(', ')}`;
          if (body.patterns.schedule?.length) result += `\n  Schedule: ${body.patterns.schedule.map((p) => `${p.pattern} (${(p.confidence * 100).toFixed(0)}%)`).join(', ')}`;
        }
    ...
    if (name === "admin_monitoring") {
      ...
      try {
        ...
        if (body.health) {
          ...
          if (subsystems.length) result += `  Subsystems: ${subsystems.map(([n, s]) => `${n}=${s.status}`).join(', ')}\n`;
        }
        ...
        if (body.goals) {
          ...
          if (body.goals.goals?.length) result += body.goals.goals.map((g) => `  • ${g.title} (${g.progress}) [${g.priority}]`).join('\n') + '\n';
        }
        ...
        if (body.activity) {
          ...
          if (body.activity.actions?.length) result += body.activity.actions.slice(0, 5).map((a) => `  • [${a.type}] ${a.description} → ${a.outcome}`).join('\n') + '\n';
        }
