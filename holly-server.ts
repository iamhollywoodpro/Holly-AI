...
async function main() {
  if (!dev) {
    ...
    // mechanism. Cold starts (~60s) are acceptable for Holly's usage pattern.
    // If cold-start latency becomes a real UX issue, the answer is a warmer
    // scaledown_window (currently 120s) — not a 24/7 keep-alive ping.
