#!/usr/bin/env/python3
"""
HOLLY FLUX.2 Klein 9B BASE (non-distilled) — A100 TEST ENDPOINT
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Purpose: Validate Klein Base (the exact base Holly's LoRAs were trained on) as
         a fix for the Distilled endpoint's geometry/anatomy limitations.
...
# ─── Klein Base (non-distilled) TEST ENDPOINT ────────────────────────────────
# Separate app + cache dir from the production Distilled endpoint
# (image_generate_flux2klein_a100.py). Purpose: validate whether running the
# FULL Klein Base model — the exact base Holly's LoRAs were trained on — fixes
# the geometry / anatomy limitations of the 4-step Distilled variant.
#
# Why Base instead of Distilled:
#   1. Holly's LoRAs are trained on FLUX.2-klein-base-9B (verified from
#      safetensors metadata: ss_sd_model_name points to base-9B). The
#      production Distilled endpoint runs FLUX.2-klein-9B — a base mismatch.
#      Switching to Base is the *correct* base for these LoRAs.
#   2. Distilled is a 4-step model that ignores CFG (its guidance signal is
#      baked in during distillation). That loss of prompt control is the
#      leading suspected cause of the finger-insertion / anatomy failures.
#   3. Base honors CFG and runs 15-50 steps, giving the sampler time to
#      resolve the geometry that 4 distilled steps cannot.
#
# This file is a TEST harness. If Base wins Steve's visual verdict, the
# production Distilled endpoint will be switched to Base; this file is then
# retired (or promoted to be the new production file). Do not run both in
# parallel long-term — they share the LoRA + encoder volumes intentionally,
# but each needs its own weight cache.
app = modal.App("holly-image-flux2klein-base")

FLUX_MODEL = "black-forest-labs/FLUX.2-klein-base-9B"
VOLUME_MOUNT = "/flux-models"
# Separate cache dir so Base weights never collide with the Distilled weights
# the production endpoint stores at /flux-models/bf16.
MODEL_CACHE = "/flux-models/bf16-base"
LORA_DIR = "/lora"
...
class HollyFlux2KleinA100:
    ...
    def load_model(self):
        ...
        self.pipe = None
        self.baked_adapters = {}
        self.model_name = "FLUX.2 Klein 9B BASE A100 (not loaded)"
        ...
        # Download FLUX.2 Klein 9B BASE BF16 to volume on first run
        try:
            ...
            if not has_index or not has_transformer:
                print(f"📥 Downloading {FLUX_MODEL} (BASE) to volume...")
                snapshot_download(
                    repo_id=FLUX_MODEL,
                ...
                volume.commit()
                print("✅ FLUX.2 Klein 9B BASE BF16 weights saved to volume")
            else:
                print("✅ FLUX.2 Klein 9B BASE BF16 weights in volume")
        ...
        try:
            print(f"🚀 Loading FLUX.2 Klein 9B BASE from {MODEL_CACHE}...")
        ...
        self.model_name = f"FLUX.2 Klein 9B BASE A100 + Baked [{adapter_list}] ({encoder_tag})"
    ...
    @modal.fastapi_endpoint(method="POST", label="generate-holly-base")
    def generate(self, request: dict):
        ...
        try:
            ...
            # Klein Base (non-distilled) defaults. Base is NOT a 4-step model —
            # the model card recommends 50 steps + CFG 4.0. 24 is a strong
            # quality/speed compromise for testing; the request may override
            # up to 50. This is the single most important difference vs the
            # Distilled endpoint, which runs 4 steps and ignores CFG.
            steps  = min(int(request.get("num_inference_steps", 24)), 50)
            seed   = request.get("seed")
            fmt    = request.get("format", "webp").lower()
            # Base honors CFG. 4.0 matches the model-card recommended value.
            ...
            return Response(
                ...
                headers={
                    ...
                    "X-Provider":    "modal-flux2klein-base",
    ...
    def _enhance_face(self, pil_img, original_prompt):
        ...
        # Sampler: Klein Base needs real steps + CFG (Distilled used 4/1.2).
        # 20 steps + CFG 4.0 on Base matches the main-generation recipe so the
        # re-rendered face lands at the same quality bar as the body.
    ...
    @modal.fastapi_endpoint(method="POST", label="inpaint-holly-base")
    def inpaint(self, request: dict):
        ...
        Request format:
            {
                ...
                "num_inference_steps": 20,   # Klein Base needs real steps (Distilled used 8)
                "guidance_scale": 4.0,
                "seed":    null,
                "format":  "webp"
            }
        ...
        try:
            ...
            width  = min(int(request.get("width",  1024)), 1024)
            height = min(int(request.get("height", 1024)), 1024)
            steps  = min(int(request.get("num_inference_steps", 20)), 50)
            seed   = request.get("seed")
            fmt    = request.get("format", "webp").lower()
            guidance_scale = float(request.get("guidance_scale", 4.0))
    ...
    @modal.fastapi_endpoint(method="GET", label="holly-health-base")
    def health(self):
        ...
        if startup_error and ("gated" in startup_error.lower() or "401" in startup_error or "403" in startup_error):
            action = "Accept gated repo license at https://huggingface.co/black-forest-labs/FLUX.2-klein-base-9B"
        return JSONResponse({
            ...
            "gpu":               "A100",
            "base_model":        "FLUX.2 Klein 9B BASE (non-distilled) BF16",
            "max_gpus":          1,
            "purpose":           "Holly Klein BASE test — validating non-distilled base for geometry/anatomy fixes",
            "trigger_word":      "h0lly",
            "licence":           "Apache-2.0",
            "version":           "1.6.0-base-test",
        })


@app.local_entrypoint()
def main():
    print("Deploy: modal deploy services/modal-media/image_generate_flux2klein_base.py")
    print("Generate: https://iamhollywoodpro--generate-holly-base.modal.run")
    print("Health:   https://iamhollywoodpro--holly-health-base.modal.run")
