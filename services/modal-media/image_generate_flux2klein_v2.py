...
# ─── Klein v2-recipe TEST ENDPOINT ───────────────────────────────────────────
# Separate app from production Distilled (image_generate_flux2klein_a100.py).
# Purpose: replicate the EXACT recipe decoded from Steve's perfect reference
# images (Civitai metadata, decoded July 28 from EXIF UserComment).
#
# The three changes vs production (all verified from decoded metadata):
#   1. BAKED body LoRA: holly-body-v2.5 → holly-body-v1 (the version the
#      perfect images used; same trigger word h0lly-body, so prompt logic is
#      unchanged)
#   2. BAKED weights: face 0.75 → 0.85, body 1.0 → 0.7 (matches decoded recipe)
#   3. Inference: 4 steps / CFG 4.0 → 12 steps / CFG 1.0 (matches decoded recipe)
#
# What is NOT changed (intentionally — preserve working systems):
#   - Base model stays FLUX.2-klein-9B (Distilled) — same as production and as
#     the perfect images (Civitai ecosystem: Flux2Klein_9B).
#   - Sampler stays the model's default (FlowMatchEulerDiscreteScheduler —
#     Euler family; this is what the perfect images used, "Euler" in Civitai UI).
#   - Same cache dir as production — Distilled weights are shared, not duplicated.
#
# Test harness. If this recipe wins Steve's visual verdict, promote to
# production. If it loses, production's locked FACT.md recipe stands unchanged.
app = modal.App("holly-image-flux2klein-v2")

FLUX_MODEL = "black-forest-labs/FLUX.2-klein-9B"
VOLUME_MOUNT = "/flux-models"
# Same cache dir as production — Distilled weights are identical, no need to
# re-download 20GB. Only the baked LoRA weights + inference params differ.
...
# v2-RECIPE (decoded from Steve's perfect reference images, Civitai metadata):
#   - Face LoRA weight 0.85 (production uses 0.75; perfect images used 0.85)
#   - Body LoRA: v1.0 file (production uses v2.5; perfect images used v1.0)
#   - Body LoRA weight 0.7 (production uses 1.0; perfect images used 0.7)
#
# NOTE: This deliberately overrides the FACT.md "locked recipe" (face 0.75 /
# body 1.0 / body v2.5) for THIS TEST ENDPOINT ONLY. Production is untouched.
# The FACT.md locked recipe was tuned for v2.5; v1.0 has different training
# characteristics and the perfect images prove v1.0 @ 0.7 + face @ 0.85 works
# better. If this test confirms it, FACT.md gets updated.
BAKED_LORAS = {
    "face": {
        "file": "holly-face-v2.safetensors",
        "weight": 0.85,
        "desc": "Holly Face v2.0 (trigger: h0lly) — v2-recipe weight 0.85 (was 0.75 in prod)",
    },
    "body": {
        "file": "holly-body-v1.safetensors",
        "weight": 0.7,
        "desc": "Holly Body v1.0 (trigger: h0lly-body) — v2-recipe: v1 file @ 0.7 (prod uses v2.5 @ 1.0)",
    },
}
...
class HollyFlux2KleinA100:
    ...
    def load_model(self):
        ...
        self.pipe = None
        self.baked_adapters = {}
        self.model_name = "FLUX.2 Klein 9B A100 v2-recipe (not loaded)"
        ...
        self.model_name = f"FLUX.2 Klein 9B A100 v2-recipe + Baked [{adapter_list}] ({encoder_tag})"
    ...
    @modal.fastapi_endpoint(method="POST", label="generate-holly-v2")
    def generate(self, request: dict):
        ...
        try:
            ...
            # v2-RECIPE: 12 steps + CFG 1.0 (matches decoded Civitai metadata from
            # Steve's perfect reference images). Production uses 4 steps / CFG 4.0
            # (Distilled's baked recipe). Request may still override up to 50 steps.
            steps  = min(int(request.get("num_inference_steps", 12)), 50)
            seed   = request.get("seed")
            fmt    = request.get("format", "webp").lower()
            # CFG 1.0 — matches decoded recipe. Production uses 4.0.
            ...
            return Response(
                ...
                headers={
                    ...
                    "X-Provider":    "modal-flux2klein-v2",
                    ...
                    "X-Version":     "1.7.0-v2-recipe-test",
        ...
        try:
            with torch.inference_mode():
                result = self.inpaint_pipe(
                    ...
                    strength=0.65,
                    # v2-recipe: 12 steps / CFG 1.0 matches main generation recipe.
                    # Production used 4 steps / CFG 1.2 (Distilled avatar recipe).
    ...
    @modal.fastapi_endpoint(method="POST", label="inpaint-holly-v2")
    def inpaint(self, request: dict):
        ...
        Request format:
            {
                ...
                "num_inference_steps": 12,   # v2-recipe (was 8 in prod)
                "guidance_scale": 1.0,       # v2-recipe (was 4.0 in prod)
                "seed":    null,
                "format":  "webp"
            }
        ...
        try:
            ...
            width  = min(int(request.get("width",  1024)), 1024)
            height = min(int(request.get("height", 1024)), 1024)
            steps  = min(int(request.get("num_inference_steps", 12)), 50)  # v2-recipe (was 8)
            seed   = request.get("seed")
            fmt    = request.get("format", "webp").lower()
            # v2-recipe: CFG 1.0 (was 4.0 in prod)
            guidance_scale = float(request.get("guidance_scale", 1.0))
    ...
    @modal.fastapi_endpoint(method="GET", label="holly-health-v2")
    def health(self):
        ...
        return JSONResponse({
            ...
            "gpu":               "A100",
            "base_model":        "FLUX.2 Klein 9B BF16 (Distilled) — v2-recipe: body v1 @ 0.7, 12 steps, CFG 1.0",
            "max_gpus":          1,
            "purpose":           "v2-recipe test — replicates Civitai recipe decoded from Steve's perfect reference images",
            "trigger_word":      "h0lly",
            "licence":           "Apache-2.0",
            "version":           "1.7.0-v2-recipe-test",
        })


@app.local_entrypoint()
def main():
    print("Deploy: modal deploy services/modal-media/image_generate_flux2klein_v2.py")
    print("Generate: https://iamhollywoodpro--generate-holly-v2.modal.run")
    print("Health:   https://iamhollywoodpro--holly-health-v2.modal.run")
