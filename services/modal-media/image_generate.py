...
@app.cls(
    ...
    scaledown_window=300,    # stay warm 5 min between requests
    timeout=300,             # 5 min max per request
    startup_timeout=600,     # 10 min — covers first-time weight download
...
class HollyImageGenerator:
    ...
    def load_model(self):
        ...
        # model_cpu_offload: keeps active sub-model on GPU, offloads rest to CPU
        # Prevents OOM on T4 (16GB) while staying faster than sequential offload
